...
from GUI.mnpbem.bem import BEMStatLayer
from GUI.mnpbem.simulation import PlaneWaveStatLayer
...
def run_case(p: ComParticle, layer: LayerStructure,
    ...
    print('[info] Python {}: BEMStatLayer loop'.format(tag))
    t0 = time.time()
...
def main() -> None:
    ...
    print('[info] === 04_bemstat_layer / sphere (Python) ===')
    print('[info] 20nm Au sphere, 1nm above glass (eps=2.25), normal + oblique(45, TM)')
    ...
    plot_spectrum(enei, [ext_n, sca_n], ['ext', 'sca'],
        'Python BEMStatLayer sphere normal (t={:.3f}s)'.format(t_n),
        os.path.join(FIG_DIR, 'normal_python.png'))
    plot_spectrum(enei, [ext_o, sca_o], ['ext', 'sca'],
        'Python BEMStatLayer sphere oblique (t={:.3f}s)'.format(t_o),
