...
def run_bem(enei: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, float]:
    ...
    for i in range(n):
        ...
        ext_x[i], ext_z[i] = ext[0], ext[1]
        sca_x[i], sca_z[i] = sca[0], sca[1]
...
def main() -> None:
    ...
    print('[info] === 02_bemstat / rod (Python) ===')
    print('[info] BEMStat trirod(10, 40, [15,15,15]) Au, x-pol + z-pol')
    ...
    plot_spectrum(enei, [ext_x, ext_z, sca_x, sca_z],
        ...
        'Python BEMStat rod (t={:.3f}s)'.format(t_bem),
    ...
    ml = load_csv(os.path.join(DATA_DIR, 'bemstat_matlab.csv'))
    if ml is not None:
