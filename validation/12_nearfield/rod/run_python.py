...
def setup_grid() -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    x_1d = np.linspace(-GRID_X, GRID_X, GRID_N)
    z_1d = np.linspace(-GRID_Z, GRID_Z, GRID_N)
...
def build_particle() -> ComParticle:
    ...
    return ComParticle(epstab, [trirod(10.0, 40.0, n = [15, 15, 15])], [[2, 1]])
...
def plot_map(e2: np.ndarray, title: str, savepath: str) -> None:
    ...
    im = ax.imshow(np.log10(e2 + 1e-30),
        extent = [-GRID_X, GRID_X, -GRID_Z, GRID_Z],
...
def main() -> None:
    ...
    print('[info] === 12_nearfield / rod (Python) ===')
    print('[info] trirod(10,40,[15,15,15]) Au, 31x31 grid at y=0, WL={}nm'.format(WL))

    e2_s, t_s = run_stat(x, y, z)
    timings['stat'] = t_s
    save_map(os.path.join(DATA_DIR, 'stat_python.csv'), x, z, e2_s)
    plot_map(e2_s, 'Python BEMStat |E|^2 rod',
    ...
    plot_map(e2_r, 'Python BEMRet |E|^2 rod',
    ...
    for case, e2_py in [('stat', e2_s), ('ret', e2_r)]:
        ...
        plot_compare(e2_py, e2_ml, '{} rod |E|^2 compare'.format(case),
