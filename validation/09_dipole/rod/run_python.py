...
from GUI.mnpbem.geometry import trirod, ComParticle, ComPoint
...
DIPOLE_POS = np.array([[0.0, 0.0, 25.0]])
...
def run_ret(wls: np.ndarray) -> Tuple[np.ndarray, np.ndarray, float]:
    p = build_particle()
    pt = ComPoint(p, DIPOLE_POS)
    dip = DipoleRet(pt, dip = DIPOLE_MOM)
    bem = BEMRet(p)
    n = len(wls)
    tot = np.zeros(n); rad = np.zeros(n)
    t0 = time.time()
...
def main() -> None:
    ...
    print('[info] === 09_dipole / rod (Python) ===')
    print('[info] trirod(10,40,[15,15,15]) Au, z-dipole at [0,0,25] (5nm above rod end)')
